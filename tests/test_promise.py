...
class WaitAndSet(Thread):
    ...
    def run(self):
        sleep(self.delay)
        self.promise.set(self.response, self.content)
...
class PromiseTest(TestCase):
    ...
    def test_done(self):
        ...
        promise.set(42, 43)
        self.assertTrue(promise.done, 'promise is done after call to set')

    def test_set_and_flag(self):
        promise = Promise()
        self.assertFalse(promise.done, 'promise does not start done')
        WaitAndSet(promise, 42, 43, 1).start()
        start = datetime.now()
        self.assertFalse(promise.done, 'promise still not done')
        self.assertEqual(promise.response, 42, 'expected response')
        self.assertEqual(promise.content, 43, 'expected content')
        duration = datetime.now() - start
        min = timedelta(microseconds=900000)
        max = timedelta(seconds=1, microseconds=100000)
